...
public class Party_stupid3 implements Party {
    final boolean _isBeta;

    private Party_stupid3(boolean isBeta) {
        _isBeta = isBeta;
    }
    ...
    public static Party New(boolean isBeta, int numDistricts, List<Block> blocks) {
        return new Party_stupid3(isBeta);
    }
    ...
    private List<Block> evenChoose(List<List<Block>> districts) {
        List<Block> lowest_win = null;
        List<Block> furthest_loss = null;
        long lowest_win_count = Long.MAX_VALUE;
        long greatest_disparity = Long.MAX_VALUE;


        for(List<Block> x : districts){
            long total = 0;
            long winning_margin = 0;
            for(Block y : x){
                total += (_isBeta) ? y.beta() : y.alpha();
                winning_margin += (_isBeta) ? y.beta() - y.alpha() : y.alpha() - y.beta();
            }

            if(winning_margin > 0){
                if(total < lowest_win_count){
                    lowest_win_count = total;
                    lowest_win = x;
                }
            }
            else{
                if(winning_margin < greatest_disparity){
                    greatest_disparity = winning_margin;
                    furthest_loss = x;
                }
            }
        }

        return (lowest_win != null) ? lowest_win : furthest_loss;
    }
